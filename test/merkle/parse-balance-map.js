...
function parseBalanceMap(balances) {
    ...
    const dataByAddress = balancesInNewFormat.reduce((memo, { address: account, earnings, reasons, index }) => {
        ...
        memo[parsed] = Object.assign({ amount: parsedNum, index }, (reasons === '' ? {} : { flags }));
        return memo;
    }, {});
    const sortedAddresses = Object.keys(dataByAddress).sort();
    // construct a tree
    const tree = new balance_tree_1.default(sortedAddresses.map((address) => ({ account: address, amount: dataByAddress[address].amount, index: dataByAddress[address].index })));
    // generate claims
    const claims = sortedAddresses.reduce((memo, address, index) => {
        const { amount, flags, index: abc } = dataByAddress[address];
        memo[address] = Object.assign({ index: abc, amount: amount.toHexString(), proof: tree.getProof(abc, address, amount) }, (flags ? { flags } : {}));
        return memo;
    }, {});
